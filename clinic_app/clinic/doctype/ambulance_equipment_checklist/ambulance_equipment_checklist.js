...
frappe.ui.form.on('Ambulance Equipment Checklist', {
	refresh: function(frm) {
		...
		frm.call('remove_all_records_on_change').then(r => {
			if(frm.doc.clinic_assets){
				frm.call('get_ambulance_asset_doc',{docname:frm.doc.clinic_assets})
			}
		})
	},yes:function(frm){
		select_all(frm,"yes")
...
frappe.ui.form.on("Ambulance Equipment Checklist Item",{
 ...
 */
 function hide_btn(btn_name){
	$('*[data-fieldname="ambulance_equipment_checklist_item"]').find(btn_name).css({
		"display": "none"
	})
}
function select_all(frm,option){
	if(option=="yes"){
		for(let item of frm.doc.ambulance_equipment_checklist_item){
			item.yes = 1
			item.no = 0
			frm.refresh_fields()
		}
